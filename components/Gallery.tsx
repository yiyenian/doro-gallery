...
export default function Gallery({ images }: { images: any[] }) {
  ...
  return (
    <>
      {/* --- 吸顶头部 (颜色修正：Slate-950) --- */}
      {/* 🔴 核心修改：bg-slate-950/90，与背景色一致，消除色差 */}
      <div className="sticky top-0 z-40 w-full bg-slate-950/95 backdrop-blur-xl border-b border-white/5 shadow-2xl transition-all">
        ...
        {/* 🔴 核心修改：w-full (移除 max-w) + px-8 (增加左右内边距) */}
        <div className="w-full px-4 sm:px-8 pt-4 pb-3">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-3">
                ...
                <div className="w-full sm:w-auto lg:w-[300px] relative group shrink-0">
                    <div className="relative flex items-center bg-black/30 rounded-lg border border-white/5 focus-within:border-indigo-500/50 focus-within:ring-1 focus-within:ring-indigo-500/50 transition-all h-9">
                    ...
                    {displayTags.map((tag) => (
                        <button 
                            ...
                            className={`px-2.5 py-0.5 rounded-md border text-[10px] font-medium transition-all duration-200 whitespace-nowrap h-[26px] flex items-center
                                ${search === tag ? 'bg-slate-100 text-slate-900 border-slate-100' : 'bg-slate-800/40 border-white/5 text-slate-400 hover:text-white hover:bg-slate-700 hover:border-white/20'}
                            `}
                        >
                            {tag}
                        </button>
