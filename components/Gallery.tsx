...
export default function Gallery({ images }: { images: any[] }) {
  ...
  return (
    <>
      {/* --- 首页 Hero 区域 (吸顶固定 + 一行流) --- */}
      <div className="sticky top-0 z-40 w-full bg-[#121212]/95 backdrop-blur-xl border-b border-white/10 shadow-2xl">
        ...
        <div className="max-w-[1960px] mx-auto px-6 pt-4 pb-3">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                ...
                <div className="flex flex-wrap items-center gap-4 text-left">
                    ...
                    <div className="flex items-center gap-2 shrink-0 select-none">
                        <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-gradient-to-tr from-indigo-600 to-violet-600 shadow-lg">
                           <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="text-white"><path d="M15 12H9V16H15C17.2091 16 19 14.2091 19 12C19 9.79086 17.2091 8 15 8H9V12"/><path d="M6 4V20" /></svg>
                        </div>
                        <span className="text-lg font-bold tracking-tight text-white font-sans">Doro</span>
                    </div>

                    {/* 竖线分割 */}
                    <div className="hidden lg:block w-px h-5 bg-white/15"></div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-baseline gap-2 sm:gap-4">
                        ...
                        {/* 描述：单行截断，鼠标悬停显示完整 */}
                        <p className="text-[11px] text-gray-500 font-medium max-w-[200px] sm:max-w-md lg:max-w-xl truncate hidden sm:block" title="High-quality AI generated imagery & prompts database. Copy, remix, create.">
                ...
                {/* 右侧：搜索框 (高度对齐) */}
                <div className="w-full lg:w-[260px] relative group shrink-0">
                    <div className="relative flex items-center bg-black/40 rounded-lg border border-white/5 focus-within:border-indigo-500/50 focus-within:ring-1 focus-within:ring-indigo-500/50 transition-all h-9">
                        ...
                        <input 
                            type="text" 
                            placeholder="Search..." 
                            className="w-full bg-transparent px-2 text-white placeholder-gray-600 focus:outline-none text-xs h-full"
                            value={search}
                        ...
                        <div className="hidden sm:flex items-center pr-2.5 pl-2 border-l border-white/5 h-4">
                            <span className="text-[9px] font-mono text-gray-500"><span className="font-bold text-indigo-400 mr-0.5">{images.length}</span></span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            {/* 第二行：Tags (全宽扩展 + 折叠) */}
            <div className="border-t border-white/5 pt-2 flex items-start gap-3 mt-2">
                <div className="text-[10px] font-bold text-gray-600 uppercase tracking-wider py-1 shrink-0 mt-px">Tags</div>
                
                {/* 🔴 核心：Tags 容器，isTagsExpanded 控制高度 */}
                {/* max-h-[26px] 刚好显示一行 */}
                <div className={`flex flex-wrap justify-start gap-2 transition-all duration-300 overflow-hidden w-full ${isTagsExpanded ? 'max-h-[500px]' : 'max-h-[26px]'}`}>
                    {displayTags.map((tag) => (
                        <button 
                            key={tag} 
                            onClick={() => setSearch(tag === search ? "" : tag)} 
                            className={`px-2 py-0.5 rounded-md border text-[10px] font-medium transition-all duration-200 whitespace-nowrap h-[26px] flex items-center
                ...
                <button 
                    ...
                    className="p-1 rounded-md bg-white/5 border border-white/10 text-gray-500 hover:text-white hover:bg-white/10 transition-colors shrink-0 h-[26px] w-[26px] flex items-center justify-center ml-auto"
