...
import { X, Copy, Check, Search, Sparkles, Image as ImageIcon, Terminal, ExternalLink } from 'lucide-react';
    ...
    <>
      ...
      {selectedId !== null && selectedImage && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
          ...
          {/* 垂直布局的弹窗主体 */}
          <div className="relative w-full max-w-5xl bg-[#0f0f11] rounded-2xl shadow-2xl ring-1 ring-white/10 flex flex-col my-auto animate-in zoom-in-95 duration-200 overflow-hidden z-50 max-h-[90vh]">
            
            {/* 1. 顶部标题栏 */}
            <div className="flex items-start justify-between p-6 border-b border-white/5 bg-[#18181b]">
                <div>
                    <h2 className="text-2xl font-bold text-white leading-tight mb-2">{selectedImage.title}</h2>
                    <div className="flex flex-wrap gap-2">
                        <span className="px-2 py-1 rounded bg-blue-500/20 text-blue-300 text-[10px] font-bold border border-blue-500/30">AI Generated</span>
                        {/* 动态标签 */}
                ...
                <div className="flex gap-2">
                   <button onClick={() => window.open(selectedImage.url, '_blank')} className="p-2 text-gray-400 hover:text-white transition bg-white/5 hover:bg-white/10 rounded-lg"><ExternalLink size={18}/></button>
                   <button onClick={() => setSelectedId(null)} className="p-2 text-gray-400 hover:text-white transition bg-white/5 hover:bg-white/10 rounded-lg"><X size={18}/></button>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8 scrollbar-thin scrollbar-thumb-gray-800">
                ...
                <div>
                    <h3 className="text-[12px] font-bold text-gray-400 uppercase tracking-wider mb-3 flex items-center gap-2">
                        <ImageIcon size={14} /> 示例图片
                    </h3>
                    <div className="rounded-xl overflow-hidden border border-white/5 bg-black/50 flex justify-center p-4">
                        <img 
