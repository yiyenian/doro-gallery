...
export default function Gallery({ images }: { images: any[] }) {
  ...
  return (
    <>
      {/* --- 紧凑版 Hero 区域 --- */}
      {/* 修改：移除了 bg-[#121212] 和 border-b，改为透明背景，减小 Padding */}
      <div className="relative pt-24 pb-8 sm:pt-28 sm:pb-12 text-center px-4 w-full bg-transparent">
        ...
        <div className="max-w-4xl mx-auto relative z-10">
            ...
            {/* 修改：字号微调，从 7xl 减小到 6xl，更协调 */}
            <h1 className="mb-6 text-4xl font-extrabold tracking-tight text-white md:text-6xl leading-tight drop-shadow-2xl">
              探索
            ...
            <div className="max-w-2xl mx-auto relative group z-10">
                ...
                <div className="relative flex items-center bg-[#18181b] rounded-full p-2 ring-1 ring-white/10 focus-within:ring-indigo-500/50 focus-within:ring-2 transition-all shadow-2xl">
                    ...
                    <input 
                        type="text" 
                        placeholder="搜索关键词..." 
                        className="w-full bg-transparent px-4 py-2.5 text-white placeholder-gray-500 focus:outline-none text-sm"
                        value={search}
            ...
            <div className="mt-6 flex flex-wrap justify-center gap-2">
                {displayTags.map((tag) => (
                    <button 
                        ...
                        className={`px-3 py-1 rounded-full border text-xs font-medium transition-all duration-300 backdrop-blur-md ${search === tag ? 'bg-white text-black border-white' : 'bg-white/5 border-white/10 text-gray-300 hover:text-white hover:bg-white/10 hover:border-white/30'}`}
                    >
                        {tag}
                    </button>
                ))}
      ...
      {/* --- 瀑布流列表 (无缝衔接) --- */}
      {/* 修改：去掉了 min-h，减小了 padding-top，让列表紧贴着 Hero 区域 */}
      ...
      {selectedId !== null && selectedImage && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
          ...
          <div className="relative w-full max-w-5xl bg-[#18181b] shadow-2xl ring-1 ring-white/10 rounded-2xl flex flex-col my-auto animate-in zoom-in-95 duration-200 overflow-hidden z-50 max-h-[90vh]">
            ...
            {/* 顶部 */}
            <div className="flex items-start justify-between p-6 border-b border-white/5 bg-[#18181b]/95 backdrop-blur-md shrink-0 z-10 sticky top-0">
            ...
            <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-transparent">
                ...
                <div className="relative w-full flex items-center justify-center py-8 px-4 group/nav">
                    <img 
                        src={selectedImage.url} 
                        className="w-auto h-auto max-h-[60vh] max-w-full object-contain shadow-2xl rounded-lg border border-white/5" 
                        alt="Detail" 
                    />
                    {selectedIndex > 0 && (
                ...
                <div className="px-4 pb-12 sm:px-8 w-full">
