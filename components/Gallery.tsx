  ...
  return (
    <div className="mb-6 last:mb-0 w-full">
      <div className="flex items-center justify-between mb-2 px-1">
        ...
        <button 
          onClick={handleCopy} 
          className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-[11px] font-bold transition-all border ${isCopied ? 'bg-green-500/10 border-green-500/20 text-green-400' : 'bg-slate-800/50 border-slate-700/50 text-slate-300 hover:bg-slate-700 hover:text-white'}`}
        >
          {isCopied ? <Check size={14}/> : <Copy size={14}/>} {isCopied ? "Copied" : "Copy"}
        </button>
      </div>
      <div className="relative group w-full rounded-xl border border-white/5 bg-[#1e293b]/50 overflow-hidden hover:border-white/10 transition-colors">
    ...
    <>
                ...
                <div className="w-full sm:w-auto lg:w-[300px] relative group shrink-0">
                    <div className="relative flex items-center bg-slate-900/50 rounded-lg border border-white/5 focus-within:border-indigo-500/50 focus-within:ring-1 focus-within:ring-indigo-500/50 transition-all h-9">
                        ...
                        <input 
                            type="text" 
                            placeholder="Search..." 
                            className="w-full bg-transparent px-2 py-1 text-white placeholder-slate-500 focus:outline-none text-xs h-full"
                            value={search}
      ...
      <div className="max-w-[1960px] mx-auto px-4 pb-20 pt-6 min-h-[100vh]">
        {filteredImages.length > 0 ? (
            ...
            {filteredImages.map((image) => (
                ...
                className="group relative mb-6 block w-full cursor-zoom-in overflow-hidden rounded-2xl bg-slate-800/40 border border-white/5 transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl hover:shadow-indigo-900/20 hover:border-white/10 backdrop-blur-sm break-inside-avoid"
                >
                <img 
                    src={image.url} 
                    ...
                    loading="lazy"
                />
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-slate-950/90 via-slate-950/60 to-transparent pt-12 pb-4 px-4">
      ...
      {selectedId !== null && selectedImage && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
          {/* 🟢 修改：遮罩 bg-slate-950/60 */}
          <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-lg transition-opacity" onClick={() => setSelectedId(null)} />
          ...
          <div className="relative w-full max-w-4xl bg-[#0f172a] shadow-2xl ring-1 ring-white/10 rounded-2xl flex flex-col my-auto animate-in zoom-in-95 duration-200 overflow-hidden z-50 max-h-[95vh]">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 bg-[#0f172a]/95 backdrop-blur-md shrink-0 z-20 sticky top-0">
                ...
                <div className="flex gap-2 shrink-0">
                   <button onClick={() => window.open(selectedImage.url, '_blank')} className="p-2 text-slate-400 hover:text-white transition bg-slate-800/50 hover:bg-slate-700 rounded-lg" title="View Original"><ExternalLink size={18}/></button>
                   <button onClick={() => setSelectedId(null)} className="p-2 text-slate-400 hover:text-white transition bg-slate-800/50 hover:bg-slate-700 rounded-lg" title="Close"><X size={18}/></button>
                </div>
            </div>
