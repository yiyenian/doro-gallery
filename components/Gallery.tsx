  ...
  return (
    <div className="mb-6 last:mb-0 w-full">
      <div className="flex items-center justify-between mb-2 px-1">
        ...
        <button onClick={handleCopy} className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-[11px] font-bold transition-all border ${isCopied ? 'bg-green-500/10 border-green-500/20 text-green-400' : 'bg-white/5 border-white/10 text-gray-300 hover:bg-white/10 hover:text-white hover:border-white/20'}`}>{isCopied ? <Check size={14}/> : <Copy size={14}/>} {isCopied ? "Copied" : "Copy"}</button>
      </div>
      <div className={`relative group w-full rounded-xl border border-white/10 bg-black/20 overflow-hidden hover:border-white/20 transition-colors cursor-pointer`} onClick={() => !isExpanded && setIsExpanded(true)}>
        <div className={`px-4 py-4 text-sm leading-7 text-gray-200 font-mono select-text whitespace-pre-wrap break-words transition-all duration-500 ease-in-out ${isExpanded ? 'max-h-[2000px]' : 'max-h-[120px] overflow-hidden'}`}>
            <ReactMarkdown components={{ p: ({node, ...props}) => <p className="mb-4 last:mb-0" {...props} /> }}>
                {content.replace(/\n/g, '  \n')}
            </ReactMarkdown>
        </div>
        {!isExpanded && <div className="absolute bottom-0 left-0 right-0 h-16 bg-gradient-to-t from-[#121212] to-transparent flex items-end justify-center pb-2 pointer-events-none"><div className="flex items-center gap-1 text-[10px] font-bold text-gray-400 bg-black/80 px-3 py-1 rounded-full border border-white/10 shadow-lg backdrop-blur-md pointer-events-auto transition-transform group-hover:scale-105 hover:text-white"><ChevronDown size={12} /> Expand</div></div>}
        {isExpanded && <div className="flex justify-center pb-4 pt-2"><button onClick={(e) => { e.stopPropagation(); setIsExpanded(false); }} className="flex items-center gap-1 text-[10px] font-bold text-gray-500 hover:text-white px-4 py-1.5 rounded-full hover:bg-white/10 transition-colors"><ChevronUp size={12} /> Collapse</button></div>}
      </div>
    </div>
  );
};
...
export default function Gallery({ images }: { images: any[] }) {
  ...
  const allTags = useMemo(() => {
    ...
    images.forEach(img => {
      if (Array.isArray(img.tags)) { img.tags.forEach((t: string) => tags.add(t)); }
    });
        ...
        <div className="max-w-[1960px] mx-auto relative z-10">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-3">
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 text-left">
                    <div className="flex items-center gap-2 shrink-0 select-none group cursor-pointer">
                        <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-tr from-indigo-600 to-violet-600 shadow-lg shadow-indigo-500/20 group-hover:scale-105 transition-transform"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="text-white drop-shadow-md"><path d="M15 12H9V16H15C17.2091 16 19 14.2091 19 12C19 9.79086 17.2091 8 15 8H9V12" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/><path d="M6 4V20" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" /></svg></div>
                    ...
                    <div className="flex flex-col justify-center">
                        <div className="flex items-baseline gap-2">
                            <h1 className="text-sm sm:text-base font-bold tracking-tight text-white/90 whitespace-nowrap">Discover <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-indigo-400 to-purple-400">Imagination</span></h1>
                        </div>
                        <p className="text-[10px] text-gray-500 font-light max-w-lg leading-tight hidden sm:block mt-0.5">High-quality AI generated imagery & prompts database.</p>
                    </div>
                </div>
                <div className="w-full sm:w-auto lg:w-[300px] relative group shrink-0">
                    <div className="relative flex items-center bg-black/40 rounded-lg border border-white/5 focus-within:border-indigo-500/50 focus-within:ring-1 focus-within:ring-indigo-500/50 transition-all h-9">
                        ...
                        <input type="text" placeholder="Search..." className="w-full bg-transparent px-2 py-1 text-white placeholder-gray-600 focus:outline-none text-xs h-full" value={search} onChange={(e) => setSearch(e.target.value)} />
                        {search && <button onClick={() => setSearch("")} className="p-1 text-gray-400 hover:text-white transition mr-1"><X size={12} /></button>}
                        <div className="hidden sm:flex items-center pr-2.5 pl-2 border-l border-white/5 h-4"><span className="text-[9px] font-mono text-gray-500 whitespace-nowrap"><span className="font-bold text-indigo-400 mr-0.5">{images.length}</span> CASES</span></div>
                    </div>
                </div>
            </div>
            <div className="border-t border-white/5 pt-3 flex items-start gap-3">
                ...
                <div className={`flex flex-wrap justify-start gap-1.5 transition-all duration-300 overflow-hidden w-full ${isTagsExpanded ? 'max-h-[500px]' : 'max-h-[28px]'}`}>
                    {displayTags.map((tag) => (
                        <button key={tag} onClick={() => setSearch(tag === search ? "" : tag)} className={`px-2.5 py-0.5 rounded-md border text-[10px] font-medium transition-all duration-200 whitespace-nowrap ${search === tag ? 'bg-white text-black border-white' : 'bg-white/5 border-white/10 text-gray-400 hover:text-white hover:bg-white/10 hover:border-white/30'}`}>{tag}</button>
                    ))}
                </div>
                <button onClick={() => setIsTagsExpanded(!isTagsExpanded)} className="p-1 rounded-md bg-white/5 border border-white/10 text-gray-400 hover:text-white hover:bg-white/10 transition-colors shrink-0 mt-0" title={isTagsExpanded ? "Collapse" : "Expand"}>{isTagsExpanded ? <ChevronUp size={12} /> : <ChevronDown size={12} />}</button>
            </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
          ...
          <div className="relative w-full max-w-4xl bg-[#18181b] shadow-2xl ring-1 ring-white/10 rounded-2xl flex flex-col my-auto animate-in zoom-in-95 duration-200 overflow-hidden z-50 max-h-[95vh]">
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 bg-[#18181b]/95 backdrop-blur-md shrink-0 z-20 sticky top-0">
                <div className="flex items-center gap-2 text-sm font-bold text-gray-400"><span className="text-indigo-400">Doro Gallery</span> / Preview</div>
