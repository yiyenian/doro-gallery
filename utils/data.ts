...
构图:居中，微距镜头视角，8K超高分辨率。`,
     ...
     "dr-43": {
    title: "3x3 3D混合时尚海报",
    tags: ["Fashion", "Collage", "3D Render", "Photography", "Grid"],
    promptCn: `创建一个3x3网格布局海报，包含9个不同的部分，并在上方叠加一个站立的裸感3D女性图像：
人物统一要求：所有场景中的女性必须是同一个人，具有相同的面部特征、发型和身材。亚洲女性，长直黑发，优雅气质，健康匀称身材。
布局结构：
底层：3列 × 3行网格，每个单元格大小相等，单元格之间有干净的白色边框（2px）
...
完整照片描述：同一位女性的全身照片，从头部到脚部完整展现。背景：阳光洒落的森林空地，温暖的自然光从侧面照射，背景有虚化的绿色植物和金色光斑，户外自然环境。视角：半侧面拍摄角度（3/4侧面视角），展现女性优雅的半侧面轮廓和身体曲线。服装：上半身穿明黄色字母印花短款露脐背心（NOMBRE3字样），紧身设计，露出腰部和腹部；下半身穿浅蓝色低腰紧身牛仔裤，从腰部到脚部，修身剪裁。姿势：双手优雅抬起轻触头发或托头，手臂自然弯曲，身体略微呈S形站姿，展现从头到脚的优美曲线。整体：运动健康身材，腰部完全露出，展现纤细腰线。
切割说明：
...
完整照片描述：一张完整的海边日落风景照片，从上到下依次包含：傍晚天空（橙粉色渐变）→ 深色海水（深蓝/暗青色，接近傍晚的深色海洋，海面平静或有轻微波纹）→ 浅色沙滩（浅米色或金色）。与第1行1-2场景相呼应，同样的海滩和日落氛围。整体营造浪漫宁静的日落氛围。
切割说明：
...
完整照片描述：同一位女性的全身照片，从头部到脚部完整展现。背景：户外自然环境，柔和日光，虚化的绿色植物和树木背景（类似公园或林间小道），自然光从侧面照射，温暖明亮的氛围。视角：右侧面拍摄角度，展现女性的右侧轮廓。服装：上半身穿深灰色或黑色无袖紧身运动背心；下半身穿黑色运动紧身裤，腰部有浅灰色或白色宽腰带装饰。姿势：侧身站立，朝向右边，面部朝向画面右侧，右手优雅向上伸展，手臂舒展呈柔美弧线，左手自然垂放。头部随着手臂动作自然后仰或侧仰，面部朝向右侧（可能闭眼或仰望），双腿自然并拢站立，身体挺直。整体：运动健康身材，从右侧面展现从头到脚的优美身体曲线。
切割说明：
