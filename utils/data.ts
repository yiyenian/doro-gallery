...
出版级分辨率，中文渲染准确，信息秩序清晰，适合专业营养科普与高端品牌使用。`,
    ...
    "dr-112": {
    title: "个人千面情绪档案：16格矩阵",
    tags: ["Portrait", "Grid", "Same Character", "Emotions", "4x4 Matrix"],
    promptCn: `这是一张极具视觉冲击力的“个人千面情绪档案”，包含 **同一位女性** 在不同生活瞬间的 16 张抓拍。
...
Tech: 35mm lens, eye-level, 8k resolution, ultra-sharp detail. --ar 9:16`,
  ...
  "dr-115": {
    title: "打工人浮生记：写实细节版",
    tags: ["Portrait", "Lifestyle", "Documentary", "Bento Grid", "Realistic", "Detailed"],
    promptCn: `【核心指令】基于上传的人物参考图，为 [插入职业名称，如：程序员/医生/建筑师] 创作一张超写实风格的 Bento 网格叙事海报。
AI智能填充：请根据职业特性，自动匹配场景、道具和细节。
人物一致性：所有画面主角必须是**同一个人**（严格锁定上传图片的脸部特征）。
【主视觉 (42%) - 早高峰】
时间 06:30。主角在通勤中（地铁/公交/步行）。
细节：眼神疲惫但坚韧。背包或手中拿着 [该职业的典型工具/文件/装备]。制服或着装符合该职业特征。
【时间轴网格 (职业生活流)】
- 08:00 准备：抵达 [该职业的工作场所]。特写：桌上/台面上摆放着 [该职业的专业设备]。
- 09:30 投入：全神贯注地 [进行该职业的核心工作]。光线真实。
- 12:00 午餐：吃着 [该职业常见的午餐形式]。视线可能还停留在工作上。
- 15:00 间隙：短暂的休息瞬间。旁边放着提神饮料。
- 18:00 晚高峰/加班：[该职业特有的加班或晚班场景]。灯光变化。
- 20:00 归途/独处：疲惫但放松的背影。
- 特写彩蛋：[该职业特有的手部特写或磨损的工具细节]。
【整体风格】
电影级纪实摄影，徕卡质感。捕捉真实的生活瞬间和职业特征。
排版：极简的白色网格分割，配以微小的无衬线时间标注。`,
  },

    "dr-116": {
    title: "超写实素描：画中画",
    tags: ["Sketch", "Graphite", "3D Render", "iPhone", "Photorealistic", "Still Life", "Reference Based"],
    promptCn: `【核心指令】基于上传的人物参考图生成。
生成一张“素描与真机对比”的超写实静物摄影图。
核心主体：
1. 白色笔记本：纸张纹理清晰，画着一位人物的超精细 3D 石墨铅笔素描。**素描中的人物面部特征必须严格基于上传的参考图绘制**，细节无可挑剔，边缘清晰。
2. iPhone：放置在笔记本旁，屏幕上显示着**用户上传的原版真人彩色参考照片**。
3. 互动：纸上的素描与手机屏幕上的人像完美对应，形成“画中画”的视觉趣味，展示从照片到素描的艺术转化。
...
35mm 胶片拍摄，8k 分辨率，照片级真实感，[品牌] 色调板，高端时尚杂志美学。`,
    ...
    "dr-125": {
    title: "历史上的今天：极简日历",
    tags: ["Calendar", "History", "Apple Design", "Minimalist", "Infographic"],
    promptCn: `指令：生成一张现代 Apple Design 风格的“历史上的今天”日历海报。
日期设定：
当前日期：[插入当前年份]年[插入月份]月[插入日期]日（星期X）。
历史事件：纪念 [插入历史年份]年[插入月份]月[插入日期]日 发生的 [插入历史事件标题]。
画面布局（竖版 4K）：
1. 顶部信息：优雅的中文大字日期 "{月}月{日}日"，旁边小字 "星期{X}"。
2. 右上角日历组件（关键准确性）：
显示 [插入当前年份]年[插入月份]月 的月历网格。
严格对齐：确保该月1号的星期位置正确（例如2026年1月1日是周四）。
高亮标注：高亮当前日期（如“6”）。
包含该月正确的天数（如1月31天）。
3. 主体卡片（磨砂玻璃质感）：
中央插画：[插入领域] 风格的干净线条艺术插画，配色克制，柔和环境光，角落有个可爱的极简卡通角色。
类别标签："历史上的今天 | [插入领域]"（细无衬线体）。
事件标题："[历史年份]年：[事件标题]"（中等字重）。
正文描述：一段关于该事件的详细描述，行距舒适 (1.8倍)。
关键词高亮：对 [插入关键词] 进行微妙强调。
设计规范 (Apple Design)：
60% 留白，8px 网格对齐，柔和投影。
晶莹剔透的排版，抗锯齿完美。
配色：背景使用 [插入领域背景色] 搭配纹理，文字 #1D1D1F，点缀色 [插入点缀色]。
水印：背景中有极细的 "[当前年份]" 英文水印，6%不透明度。
底部右侧："创作者: Doro"。
画质：4K超高清，极简、优雅、精致。`,
  },
...
**Tech:** Close-up framing, tight on face, hyper-detailed, cinematic clarity, soft film grain. Poster-like composition, timeless. --ar 3:4 --v 6.0`,
    ...
    "dr-131": {
    title: "宝丽来散落拼图：私密时刻",
    tags: ["Portrait", "Polaroid", "Flat Lay", "Lifestyle", "Collage"],
    promptCn: `【核心指令】基于上传参考图生成一张超写实俯拍静物摄影。
主体内容：
5张复古宝丽来(Polaroid)照片自然散落在背景上，照片中有轻微重叠。
**人物锁定：** 所有照片中的人物必须是**同一位年轻亚洲女性**（严格锁定上传图的面部特征，面部不进行任何改变）。
人物造型：黑色波浪卷发，白皙皮肤，清透妆容，身穿黑色背心，佩戴金色吊坠项链。
照片内容规划：
1. 左上：眨眼，吐舌头，比耶手势（顽皮）。
2. 右上：飞吻动作（甜蜜）。
3. 中间：看向画外，手托腮（若有所思）。
...
5. 右下：开怀大笑，歪头（自然）。
环境背景：
背景是一条厚实的奶油色针织羊毛毯，具有清晰的编织纹理。
关键细节：毯子上直接印有几个红色的口红唇印（Kiss marks），增添私密感。
光影与画质：
整体光线：柔和的顶部自然环境光（Daylight）。
照片内光线：模拟直闪风格（Studio flash），明亮且带有柔和阴影，略微过曝的胶片感。
画质：8K超高清，宝丽来相纸质感（白边、轻微磨损），对焦锐利。`,
  },

    "dr-132": {
    title: "丝线绘像：超写实刺绣人像",
    tags: ["Embroidery", "Portrait", "Textile Art", "Realistic", "Craft"],
    promptCn: `【核心指令】基于上传的人物参考图生成一张超写实的手工刺绣艺术作品。
人物锁定：严格使用上传图片作为角色参考。必须 100% 保留人物的面部结构、表情、肤色、发型、服装、比例和姿势。
严格限制：除了进行刺绣材质的转化外，不得修改、美化、风格化或重新诠释面部特征。
工艺细节：
- 将人物渲染为极致细节的手工刺绣艺术品。
- 针法：使用写实的缎面绣（Satin Stitch）、长短针（Long-and-short Stitch）和密集的填充针法。
- 质感：通过丝线的层叠创造自然的深度、织物纹理和丝线特有的光泽感。
- 走线：可见的绣线纹理，针脚方向精确跟随面部轮廓和肌肉走向，形成平滑的色彩渐变。
- 品质：博物馆级的纺织工艺质感，高针数密度。
布局补充（原图对比）：
在画面左下角，叠加显示原始参考照片的小图（Inset Image），形成刺绣成品与原图的直观对比。
环境与光影：
背景：中性织物背景。
光影：柔和的影棚布光，强调丝线的立体感和光泽。
画质：超锐利的微距细节，照片级刺绣真实感。拒绝插画或卡通风格。`,
  },

    "dr-133": {
    title: "小朋友每日学个字：极简日历",
    tags: ["Education", "Calendar", "Chinese Character", "Kids", "Apple Design"],
    promptCn: `【指令】生成一张现代 Apple Design 风格的“小朋友每日学个字”日历海报 (4K竖版)。
日期：{月}月{日}日，{年份}。汉字："{汉字}" (主题：{主题分类})。
【画面布局】
1. 顶部：左侧大号日期+星期，下方优雅展示年份“— {年份} —”。右侧为{年份}年{月}月日历组件（**关键：**网格必须准确，1号星期对齐正确，高亮当前日期，天数正确）。
2. 核心展示：中央巨大、美观的楷书"{汉字}"（无笔顺编号），带有柔和主题色光晕。下方标准田字格（描红引导）。
3. 主题插画：根据{主题分类}设计的可爱卡通场景（如动物/植物/生活），风格圆润亲切。右下角或场景中包含一只可爱的{动物}吉祥物老师（如戴眼镜的熊猫/兔子），配有鼓励气泡。
4. 教育信息卡片（磨砂玻璃质感）：
   - 拼音（带声调）、笔画数（准确）、部首、结构。
   - 释义：儿童易懂的简单解释。
   - 例句：生活化句子 + Emoji。
   - 小知识：趣味记忆点。
   - 关键词高亮显示。
5. 底部信息：左侧“适合 {年龄范围}”胶囊标签，右侧“创作者: doro”优雅小字。
【设计规范】
风格：极简、高级、亲和力强。60%留白，8px网格系统，柔和投影。
配色：奶白/柔白背景 + {主题色}点缀 + 深灰文字。背景有极细微纸张纹理。
字体：圆润无衬线体（正文）+ 优雅衬线体（年份）。`,
  },

    "dr-134": {
    title: "九宫格新年：斑马派对",
    tags: ["New Year", "Grid", "Portrait", "Zebra", "Color Block", "Commercial"],
    promptCn: `基于参考图片生成一张【九宫格新年主题人像海报】，整体为 3×3 九宫格排版。
核心指令：人物与“马”元素保持一致，其中“马”统一为【斑马】，不得替换为其他动物。人物面部特征、五官比例、肤色、发型与整体气质必须在九宫格中完全一致，不换脸、不改变人物身份。
整体设定
人物始终坐在【超大的毛绒斑马玩具堆中】，斑马为核心视觉元素。
**关键要求：每个格子里的斑马形态、大小或姿态必须截然不同，拒绝重复的斑马造型。** 斑马材质为真实毛绒质感，条纹清晰、立体蓬松。
人物下方统一铺设一层【纯白色毛茸茸皮草地毯】，质感柔软厚实。
九宫格内容变化规则（关键）：
- **9张完全不同**：9个格子中的人物必须展现9种**截然不同**的表情和动作组合，严禁重复。
- **动作示例**：双手张开欢呼、单手托腮思考、双手捂脸害羞、比心手势、紧抱斑马、俏皮敬礼、指镜头互动、整理头发、闭眼许愿等。
- **表情示例**：大笑、微笑、惊讶、搞怪吐舌、温柔注视、调皮眨眼等，整体保持开心喜悦的氛围。
背景与文字规则：
- 背景为【纯枣红色 / 深红偏暖】，颜色饱满、简洁纯粹、无杂质。
- **唯一文字**：“新年快乐”白色书法字体**仅保留一个**，作为一个视觉重心出现在画面中（例如中间格或作为整体装饰），**绝对不要在每个格子里重复出现**。
- 画面右上角可选择性加入【红色印章样式装饰】，用于增强中国新年仪式感。
整体风格：
...
总体排版：信息极度密集，被组织成6-8个结构化的模块，但通过中心周围战略性的空余取得平衡，让写实的中心主体脱颖而出。硬核、专业、学术，同时通过动态3D中心主体保持视觉吸引力。无任何百科品牌标志。`,
    ...
    "dr-173": {
    title: "儿童涂色手抄报：上下同构",
    tags: ["Children's Book", "Coloring Page", "Template", "Fable", "Education", "Line Art"],
    promptCn: `核心指令：你是一名专业的儿童插画师。首先严格考证并理解主题 [填写成语或寓言名称，例如：虎口拔牙] 的真实故事情节与角色关系（例如“虎口拔牙”是狐狸给老虎拔牙，必须保证故事主角与情节绝对准确）。在此基础上，生成一张儿童绘本风的手抄报模板。画面采用竖版上下分屏构图：上半部分为彩色插画，下半部分为与上半部分构图完全一致的黑白线稿（供儿童涂色）。
标题区：顶部居中排版可爱的卡通手写字中文标题。每个汉字分别放入蓝、紫、绿、黄交替的高饱和但柔和的彩色圆形气泡中。
场景设定：基于考证后的准确故事情节，左侧为主角色 [填写故事真实主角1及其动作表情]，右侧为次角色 [填写真实主角2及其动作表情]，形成准确的故事互动与对比。中间有弯曲道路或河流作为关键视觉引导。环境包含树木、草地等，风格简洁且儿童友好。
构图细节：在画面右侧保留充足的留白区域，并画上横线用于儿童书写。左右画面保持平衡，留白充足。周围带有干净规整的中等粗细蓝色描边边框。
视觉风格：上半部分色彩明亮柔和、低对比度，有轻微渐变的儿童绘本质感。线条圆润、粗细均匀、清晰描边。整体具有童趣、轻松和教育意义。
...
Constraints: All elements must strongly bind to the theme, instantly recognizable. No clutter, no hard patchwork, no templated backgrounds, no cheap fantasy assets. --ar 5:7 --v 6.0 --style raw`,
    ...
    "dr-176": {
    title: "城市十二时辰：3x4网格海报",
    tags: ["Poster", "Cityscape", "Collage", "Grid Layout", "Time Transition", "Typography"],
    promptCn: `核心指令：为【[填写城市名]】创作一张高完成度、高审美、适合系列化发布的「【[填写城市名]】十二时辰」主题竖版海报。这不是普通旅游拼图或风景海报，而是一张具有“时间叙事感、城市识别度、系列封面感、杂志专题感”的主题海报，画面需精致、统一、清晰且具传播感。
版式与画幅结构（严格约束）：固定为竖版海报构图，比例3:4，具有大海报感与高分辨率质感。整张海报必须由12张相对独立又彼此统一的图片组成，采用固定的“3列×4行”结构排布，形成清晰、精致的拼贴网格。绝不能像杂乱无章的截图拼接。
主题逻辑与内容要求：围绕该城市展开，形成从日出到日落、再到夜幕降临的完整时间线（如晨曦、正午、蓝调时刻、夜景），体现一天之内流动感、节奏感和层次感。必须自然分布该城市最具代表性的重点地标、天际线、人文街区或水域，避免机械重复或只拍同一建筑。
视觉风格与色彩光影：整体以真实摄影感、文旅专题感、城市杂志感为主。画面干净有秩序，兼具纪录片气质与轻微电影感（忌过度修饰失真）。色彩必须随时间线自然变化形成连续的色温过渡（日出暖金到入夜蓝调），但后期调色风格仍需保持整体系列感的统一。强调天空、水面、建筑灯光与空气感。
标题设计与排版重心：主标题为「【[填写城市名]】十二时辰」，是视觉锚点。必须通过高级方式处理文字可读性（如合理留白、局部暗化渐变、柔和虚化或光影避让），绝不能简单粗暴地在文字后垫纯黑色块。阅读逻辑需清晰：先看标题，再浏览12个时间切片。可适度搭配小面积副标题、时间标签或印章。
自适应补全：如果只输入城市名，请自动理解并补全该城市最具代表性的地标、最适合表现的一天时间线、最典型场景及光影色彩变化。避免杂乱失控、文字生硬压图及低质旅游宣传感。`,
    promptEn: `Core Instruction: Create a highly finished, highly aesthetic vertical poster themed "Twelve Hours of [[Fill in City Name]]", suitable for serialized publishing. This is not a simple travel collage but a thematic poster with "narrative of time, city recognizability, series cover feel, and magazine editorial vibe", requiring an exquisite, unified, and shareable look.
Layout & Canvas Structure (Strict Constraints): Fixed vertical composition with a 3:4 aspect ratio, featuring a grand poster feel and high-resolution texture. The poster MUST consist of exactly 12 individual yet unified images, arranged in a strict "3 columns by 4 rows" grid, forming a clear and exquisite collage. Absolutely no messy screenshot patchwork.
Theme Logic & Content Requirements: Revolving around the single city, forming a complete timeline from sunrise to sunset to nightfall (e.g., dawn, noon, blue hour, night views), reflecting the fluidity, rhythm, and layers of a day. Naturally distribute the city's most representative landmarks, skylines, cultural blocks, or waterscapes without mechanical repetition.
Visual Style, Color & Lighting: Authentic photography feel, cultural tourism editorial vibe, city magazine aesthetic. Clean, orderly, combining documentary temperament with a slight cinematic touch (avoid over-editing). Colors MUST shift naturally along the timeline (warm golden sunrise to blue hour nightfall) while maintaining a unified post-processing tone for series cohesion. Emphasize sky, water reflections, architectural lights, and aerial perspective.
Typography & Visual Focus: The main title "Twelve Hours of [[Fill in City Name]]" acts as the visual anchor. Use advanced techniques for text legibility (strategic negative space, localized darkening gradients, soft blurring, or dodging light). DO NOT use crude solid black boxes behind the text. Clear reading logic: see title first, then the 12 time slices. Can pair with small subtitles, time tags, or seals.
Adaptive Completion: If only the city name is provided, automatically fill in the most representative landmarks, ideal daily timeline, typical scenes, and logical lighting/color shifts. Avoid clutter, chaotic collages, rigid text-over-image, and cheap tourist promo vibes. --ar 3:4 --v 6.0 --style raw`,
    prompt: "Highly aesthetic vertical poster of [City Name], strict 3 columns by 4 rows (3x4) grid collage layout exactly 12 images. Time narrative progression from sunrise to nightfall. Featuring iconic city landmarks, skylines, cultural streets. Natural timeline color transition: warm gold to crystal day to orange dusk to blue hour night city lights. Unified premium travel magazine color grading, authentic photographic texture, atmospheric perspective. Elegant typography title 'Twelve Hours in [City Name]' blending naturally with advanced text visibility techniques, NO crude black background boxes behind text, minimal exquisite time stamps. Neat, organized, cinematic, documentary vibe, masterpiece, 8k resolution --ar 3:4 --v 6.0 --style raw",
  },

    "dr-177": {
    title: "新中式写实：双重曝光海报",
    tags: ["Poster", "Neo-Chinese", "Double Exposure", "Cinematic", "Narrative", "Silhouette"],
    promptCn: `核心指令：请根据我提供的【[填写主题/IP名称，例如：牡丹亭/繁花/大话西游/霸王别姬]】，自动匹配最符合该主题的核心人物与标志性场景，生成一张高审美、具备【新中式写实主义】（或最契合该主题的东方写实美学）风格的叙事性艺术海报。
核心构图：采用大面积留白与中央双重曝光主体。以该主题中【最具代表性的核心人物】的侧脸剪影作为视觉容器，剪影内部自动生长出【最契合该主题的多层次标志性场景与意象组合】（如经典建筑、名场面群像、专属道具或环境）。利用建筑或环境结构形成极具空间纵深感的视觉引导线。
视觉风格与色彩：底色采用做旧的灰白色宣纸或肌理纸质感。整体色调由AI根据主题气质自动提取，但统一处理为低饱和度的大地色系、水墨灰与暗金色调。通过强烈的明暗对比（Chiaroscuro），让局部暖黄色的灯火破开冷峻灰暗的基调。
灯光与特效：内部嵌套场景带有电影感的局部点光源（如红灯笼、烛光、环境暖光）。画面边缘柔化，呈现烟雾缭绕的氤氲感、双重曝光的透明度渐变，以及微弱的电影胶片颗粒感。
排版美学与氛围：画面采用垂直居左（或极简留白）排版，使用古典书法字体作为主标题，现代极简无衬线体作为副标题与英文，并可点缀一枚红色朱砂印章以增加传统文化背书感。整体传递出完美契合该主题的宿命、哀愁、如梦似幻的史诗级叙事氛围。`,
    promptEn: `Core Instruction: Based solely on the theme/IP [Subject, e.g., The Peony Pavilion / Blossoms / A Chinese Odyssey], automatically match the most representative core character and iconic scenes to generate a highly aesthetic narrative art poster in the [Neo-Chinese Realism] style (or matching Oriental realism aesthetics).
Core Composition: Features large negative space and a central double-exposure subject. Use the profile silhouette of the theme's most iconic character as a visual container. Inside the silhouette, automatically nest multi-layered narrative scenes and symbolic elements that perfectly represent the theme's worldview (e.g., classic architecture, memorable group scenes, specific props). Use architectural or environmental structures to create visual leading lines with deep spatial depth.
Visual Style & Colors: Aged off-white rice paper or textured paper background. The color palette is automatically extracted from the theme's vibe by the AI, but uniformly processed into low-saturation earth tones, ink gray, and dark gold. Utilizes strong chiaroscuro, where localized warm yellow lights break through the cold, stark gray base tone.
Lighting & Effects: Nested scenes feature cinematic local point lights (e.g., glowing lanterns, candlelight, warm ambient light). Softened edges, smoky and misty atmosphere, seamless double-exposure opacity gradients, and subtle cinematic film grain.
Typography & Atmosphere: Vertical left-aligned minimalist typography, featuring classical Chinese calligraphy for the main title and modern minimalist sans-serif for subtitles/English, accented with a red cinnabar seal for cultural depth. The overall atmosphere conveys a fateful, sorrowful, dreamlike, and epic narrative mood perfectly fitting the theme. --ar 3:4 --v 6.0 --style raw`,
    prompt: "Neo-Chinese realism narrative art poster about [Subject], double exposure. Profile silhouette of the most iconic character from [Subject] acting as a visual container, automatically nesting multi-layered iconic scenes, memorable moments, and symbolic elements specific to [Subject], deep spatial depth. Aged off-white rice paper background, low-saturation earth tones matching the theme's vibe, ink gray, dark gold, strong chiaroscuro. Cinematic warm yellow local lighting breaking through cold base tone. Vertical left-aligned Chinese calligraphy title, modern sans-serif subtitles, red cinnabar seal. Smoky misty atmosphere, soft edges, double-exposure opacity gradient, film grain. Fateful, sorrowful, dreamlike epic narrative mood perfectly matching [Subject], masterpiece, 8k resolution, highly detailed --ar 3:4 --v 6.0 --style raw",
  },
  
    "dr-178": {
    title: "东方水墨：卷轴剪影叙事海报",
    tags: ["Poster", "Oriental", "Ink Wash", "Silhouette", "Scroll Art", "Traditional"],
    promptCn: `核心指令：请围绕【[填写主题/灵感/作品要求]】，创作一张具有收藏价值的东方叙事手绘海报。整体风格必须接近高级国风纸本插画、东方水墨淡彩叙事绘画、收藏版书卷海报的质感。
核心构图：画面主体为一个巨大、完整、优雅、极具识别度的人物侧脸剪影。该侧脸像一笔气韵连贯的主形，轮廓与内部世界自然相生。剪影内部从主题中提炼最核心的叙事场景、角色关系、象征符号与建筑氛围，让它们像山海云气般自然浮现、层层展开，形成卷轴式、章节式的东方叙事节奏。整体结构要像同一位顶级插画师在同一张纸上统一经营绘制的画作，而非机械合成。
视觉风格与绘画语言：偏东方手绘、半写意、非写实。重点表达气韵、虚实、留白、层次和诗性。山石云雾建筑可清晰但不应过分写实。保留明显的手绘概括感（有精细、有轻带、有融入云气），呈现“可读、可看、可品”的东方叙事绘画质感。
质感与色彩：必须是纸本手绘感，克制地表现淡墨晕染、赭石淡彩、干刷痕迹、边缘飞白、轻微渗化及旧纸底纹，服务于叙事节奏。色彩系统高度克制，以低饱和、纸本感、东方古意为主（米白、淡墨灰、赭石、灰棕、烟金），加极少量点睛色。色彩安静、统一、含蓄，像从古画中生长出来。
排版与专属签名：保留大面积自然留白。标题采用竖排中文书法或题签式排版，辅助文字极少量、细长排布如书页边注。画面中必须包含专属签名“xxd”，自然融入整体版式，像作者落款、题签或小型印章，低调清晰不破坏气韵。
负面避免：写实摄影感、3D渲染感、欧美奇幻概念图、过度细节堆砌、强材质真实感、商业电影海报光效、机械双重曝光拼贴、现代潮流字体、廉价假水彩特效。`,
    promptEn: `Core Instruction: Create a highly collectible Oriental narrative hand-drawn poster based on [Subject/Inspiration]. The style must resemble premium traditional Chinese paper illustrations, Oriental ink wash light-color narrative paintings, and scroll-like art posters.
Core Composition: The main subject is a giant, elegant, highly recognizable profile silhouette of the core character. The silhouette acts as a continuous stroke of Qi (spiritual energy), naturally blending with its internal world. Inside, extract core narrative scenes, character relationships, and architectural motifs, letting them emerge naturally like clouds and mountains, forming a scroll-like, episodic Oriental narrative rhythm. The structure must look organically drawn by a master illustrator on a single sheet of paper, not a mechanical photo-bash.
Visual Style & Language: Oriental hand-drawn, semi-freehand (Xieyi), non-realistic. Focus heavily on artistic conception (Qiyun), solid vs. void, negative space, and poetic layers. Retain clear hand-drawn generalization (detailed in some parts, lightly brushed in others).
Texture & Color: Authentic paper hand-drawn texture. Restrained use of ink wash bleeding, light ochre, dry brush marks, flying white edges, and antique paper grain to serve the atmosphere. Extremely restrained color palette: low-saturation, paper-like Oriental antiquity (off-white, light ink gray, ochre, smoky gold) with minimal accent colors. Quiet, unified, and elegant.
Typography & Signature: Large areas of natural negative space. Vertical Chinese calligraphy titles, small delicate marginal notes. Must include the exclusive signature "xxd" naturally integrated as an artist's colophon, seal, or signature, low-key but clear.
